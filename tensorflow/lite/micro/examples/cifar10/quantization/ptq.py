...
`bazel-bin/tensorflow/lite/micro/examples/cifar10/train --save_tf_model --save_dir=/tmp/float_model/`
...
The quant model (named cifar10_int8.tflite) will be created inside the target_dir.
`bazel-bin/tensorflow/lite/micro/examples/cifar10/quantization/ptq  --source_model_dir=/tmp/float_model --target_dir=/tmp/quant_model/`
`
bazel-bin/tensorflow/lite/micro/examples/cifar10/quantization/ptq\
   --source_model_dir=/workspace/tflm/tflite-micro/tensorflow/lite/micro/examples/cifar10/models/cifar10 \
   --target_dir=/workspace/tflm/tflite-micro/tensorflow/lite/micro/examples/cifar10/models
`
"""
import math
import os
...
def convert_quantized_tflite_model(source_model_dir, x_values):
  """Convert the save TF model to tflite model, then save it as .tflite
    ...
    Args:
        source_model_dir (tf.keras.Model): the trained cifar10 flaot Model dir
...
def main(_):
  ...
  save_tflite_model(quantized_tflite_model,
                    FLAGS.target_dir,
                    model_name="cifar10_int8.tflite")
